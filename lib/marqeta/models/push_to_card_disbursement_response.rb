...
module Marqeta
  class PushToCardDisbursementResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::PushToCardDisbursementResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::PushToCardDisbursementResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@payment_instrument_token.nil? && @payment_instrument_token.to_s.length > 36
      ...
      if !@payment_instrument_token.nil? && @payment_instrument_token.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@payment_instrument_token.nil? && @payment_instrument_token.to_s.length > 36
      return false if !@payment_instrument_token.nil? && @payment_instrument_token.to_s.length < 1
    ...
    def hash
      [created_time, last_modified_time, status, token, currency_code, amount, payment_instrument_token, tags, memo].hash
    end
