  ...
  class AcceptedCountriesModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::AcceptedCountriesModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::AcceptedCountriesModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_whitelist')
        self.is_whitelist = attributes[:'is_whitelist']
      else
    ...
    def list_invalid_properties
      ...
      if @is_whitelist.nil?
        invalid_properties.push('invalid value for "is_whitelist", is_whitelist cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @is_whitelist.nil?
    ...
    def hash
      [token, name, is_whitelist, country_codes, created_time, last_modified_time].hash
    end
