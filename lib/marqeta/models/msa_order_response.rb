  ...
  class MsaOrderResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::MsaOrderResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::MsaOrderResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [token, user_token, business_token, order_balances, purchase_amount, last_transaction_date, start_date, end_date, currency_code, active, reward_amount, reward_trigger_amount, unloaded_amount, campaign_token, funding, created_time, last_modified_time, aggregated_balances, transaction_token].hash
    end
