...
module Marqeta
  class AuthControlExemptMidsResponse
    attr_accessor :token
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::AuthControlExemptMidsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::AuthControlExemptMidsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @mid.to_s.length > 36
        invalid_properties.push('invalid value for "mid", the character length must be smaller than or equal to 36.')
      end
    ...
    def valid?
      ...
      return false if @mid.to_s.length > 36
    ...
    def mid=(mid)
      ...
      if mid.to_s.length > 36
        fail ArgumentError, 'invalid value for "mid", the character length must be smaller than or equal to 36.'
      end
    ...
    def hash
      [token, name, association, mid, start_time, end_time, active, created, last_updated].hash
    end
