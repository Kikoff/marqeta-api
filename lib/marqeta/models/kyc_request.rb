  ...
  class KycRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::KycRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::KycRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'manual_override')
        self.manual_override = attributes[:'manual_override']
      else
    ...
    def list_invalid_properties
      ...
      if !@reference_id.nil? && @reference_id.to_s.length > 32
        invalid_properties.push('invalid value for "reference_id", the character length must be smaller than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if !@reference_id.nil? && @reference_id.to_s.length > 32
    ...
    def reference_id=(reference_id)
      if !reference_id.nil? && reference_id.to_s.length > 32
        fail ArgumentError, 'invalid value for "reference_id", the character length must be smaller than or equal to 32.'
      end
    ...
    def hash
      [notes, token, user_token, business_token, manual_override, reference_id].hash
    end
