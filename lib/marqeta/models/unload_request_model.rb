  ...
  class UnloadRequestModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::UnloadRequestModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::UnloadRequestModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @original_order_token.to_s.length > 36
        invalid_properties.push('invalid value for "original_order_token", the character length must be smaller than or equal to 36.')
      end

      if @original_order_token.to_s.length < 1
        invalid_properties.push('invalid value for "original_order_token", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @original_order_token.to_s.length > 36
      return false if @original_order_token.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] original_order_token Value to be assigned
    def original_order_token=(original_order_token)
      if original_order_token.nil?
        fail ArgumentError, 'original_order_token cannot be nil'
      end

      if original_order_token.to_s.length > 36
        fail ArgumentError, 'invalid value for "original_order_token", the character length must be smaller than or equal to 36.'
      end

      if original_order_token.to_s.length < 1
        fail ArgumentError, 'invalid value for "original_order_token", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [token, original_order_token, amount, tags, memo, funding_source_address_token].hash
    end
