...
module Marqeta
  class InternalAuthorizationTransaction
    attr_accessor :returned_balances

    attr_accessor :tags

    attr_accessor :fees_model
    ...
    def self.openapi_types
      {
        ...
        :'tags' => :'String',
        :'fees_model' => :'Array<NetworkFeeModel>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::InternalAuthorizationTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::InternalAuthorizationTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'fees_model')
        if (value = attributes[:'fees_model']).is_a?(Array)
          self.fees_model = value
        end
      end
    ...
    def hash
      [returned_balances, tags, fees_model, transaction_type, token, ref_transaction, node, network, sub_network, mid, tid, stan, ca_name, ca_street, ca_zip, ca_city, ca_region, ca_country, function_code, reason_code, response_code, approval_number, display_message, date, transmission_date, local_transaction_date, capture_date, settlement_date, itc, irc, currency_code, amount, additional_amount, acquirer_fee, issuer_fee, rc, extrc, duration, cardholder, acting_cardholder, card, account, account2, mcc, network_reference_id, acquirer_reference_id, retrieval_reference_number, forwarding_inst_id, network_mid, request_amount, transaction_state, remote_host, response_amount, expiration_time, incoming_network_request_itc, digital_wallet_token, tranlog_attributes, payload, layer, transaction_name, originator, acquirer].hash
    end
