  ...
  class TokenRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::TokenRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::TokenRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @cvv_number.nil?
        invalid_properties.push('invalid value for "cvv_number", cvv_number cannot be nil.')
      end

      if @cvv_number.to_s.length > 4
        invalid_properties.push('invalid value for "cvv_number", the character length must be smaller than or equal to 4.')
      end

      if @cvv_number.to_s.length < 3
        invalid_properties.push('invalid value for "cvv_number", the character length must be great than or equal to 3.')
      end
    ...
    def valid?
      ...
      return false if @cvv_number.nil?
      return false if @cvv_number.to_s.length > 4
      return false if @cvv_number.to_s.length < 3
    ...
    # Custom attribute writer method with validation
    # @param [Object] cvv_number Value to be assigned
    def cvv_number=(cvv_number)
      if cvv_number.nil?
        fail ArgumentError, 'cvv_number cannot be nil'
      end

      if cvv_number.to_s.length > 4
        fail ArgumentError, 'invalid value for "cvv_number", the character length must be smaller than or equal to 4.'
      end

      if cvv_number.to_s.length < 3
        fail ArgumentError, 'invalid value for "cvv_number", the character length must be great than or equal to 3.'
      end
    ...
    def hash
      [token, user_token, business_token, account_number, cvv_number, exp_date, zip, postal_code, is_default_account].hash
    end
