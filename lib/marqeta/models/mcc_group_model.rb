  ...
  class MccGroupModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::MccGroupModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::MccGroupModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'mccs')
        if (value = attributes[:'mccs']).is_a?(Array)
          self.mccs = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @mccs.nil?
        invalid_properties.push('invalid value for "mccs", mccs cannot be nil.')
      end
    ...
    def hash
      [token, name, mccs, active, config].hash
    end
