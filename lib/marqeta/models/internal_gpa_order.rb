  ...
  class InternalGPAOrder
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::InternalGPAOrder` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::InternalGPAOrder`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'response_memo')
        self.response_memo = attributes[:'response_memo']
      end
    ...
    def valid?
      ...
      order_state_validator = EnumAttributeValidator.new('String', ["PENDING", "REVERSED", "CLEARED", "COMPLETION", "DECLINED", "ERROR"])
    ...
    def order_state=(order_state)
      validator = EnumAttributeValidator.new('String', ["PENDING", "REVERSED", "CLEARED", "COMPLETION", "DECLINED", "ERROR"])
    ...
    def hash
      [tags, memo, token, amount, currency, funding_source, original_order_id, order_type, order_state, response_code, response_memo, created_time, last_modified_time].hash
    end
