  ...
  class UserCardHolderResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::UserCardHolderResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::UserCardHolderResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [authentication, token, active, honorific, gender, first_name, middle_name, last_name, email, address1, address2, city, state, zip, postal_code, country, notes, phone, parent_token, uses_parent_account, ssn, corporate_card_holder, passport_number, id_card_number, nationality, company, ip_address, password, created_time, last_modified_time, business_token, metadata, account_holder_group_token, status, identifications, deposit_account, birth_date, passport_expiration_date, id_card_expiration_date].hash
    end
