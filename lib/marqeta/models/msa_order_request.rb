  ...
  class MsaOrderRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::MsaOrderRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::MsaOrderRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @funding_source_token.to_s.length < 1
    ...
    def valid?
      ...
      return false if @funding_source_token.to_s.length < 1
    ...
    def hash
      [token, campaign_token, user_token, business_token, currency_code, purchase_amount, reward_amount, reward_trigger_amount, start_date, end_date, funding_source_token, funding_source_address_token].hash
    end
