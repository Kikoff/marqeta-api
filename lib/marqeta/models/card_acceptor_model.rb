  ...
  class CardAcceptorModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::CardAcceptorModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::CardAcceptorModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@mcc.nil? && @mcc.to_s.length < 0
        invalid_properties.push('invalid value for "mcc", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@mcc.nil? && @mcc.to_s.length < 0
    ...
    def mcc=(mcc)
      ...
      if !mcc.nil? && mcc.to_s.length < 0
        fail ArgumentError, 'invalid value for "mcc", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [mcc, partial_approval_capable, name, address, city, state, zip, country, ecommerce_security_level_indicator].hash
    end
