...
module Marqeta
  class DirectDepositAccountTransitionRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::DirectDepositAccountTransitionRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::DirectDepositAccountTransitionRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @account_token.to_s.length < 1
        invalid_properties.push('invalid value for "account_token", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @account_token.to_s.length < 1
    ...
    def account_token=(account_token)
      ...
      if account_token.to_s.length < 1
        fail ArgumentError, 'invalid value for "account_token", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [account_token, state, channel, reason, token].hash
    end
