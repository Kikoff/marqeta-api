  ...
  class SpendControlAssociation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::SpendControlAssociation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::SpendControlAssociation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@card_product_token.nil? && @card_product_token.to_s.length > 36
        invalid_properties.push('invalid value for "card_product_token", the character length must be smaller than or equal to 36.')
      end

      if !@card_product_token.nil? && @card_product_token.to_s.length < 1
    ...
    def valid?
      return false if !@card_product_token.nil? && @card_product_token.to_s.length > 36
      return false if !@card_product_token.nil? && @card_product_token.to_s.length < 1
    ...
    def hash
      [card_product_token, user_token].hash
    end
