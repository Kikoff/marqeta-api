...
module Marqeta
  class DirectDepositTransitionRequest
    attr_accessor :token
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::DirectDepositTransitionRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::DirectDepositTransitionRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @direct_deposit_token.nil?
        invalid_properties.push('invalid value for "direct_deposit_token", direct_deposit_token cannot be nil.')
      end
    ...
    def valid?
      ...
      channel_validator = EnumAttributeValidator.new('String', ["API", "SYSTEM", "PROD_SUPPORT"])
      return false unless channel_validator.valid?(@channel)
      return false if @direct_deposit_token.nil?
      return false if @state.nil?
      state_validator = EnumAttributeValidator.new('String', ["PENDING", "APPLIED", "REVERSED", "REJECTED"])
      return false unless state_validator.valid?(@state)
      reason_code_validator = EnumAttributeValidator.new('String', ["R01", "R02", "R03", "R04", "R06", "R08", "R09", "R10", "R11", "R14", "R15", "R16", "R17", "R18", "R23", "R24", "R29"])
    ...
    def channel=(channel)
      validator = EnumAttributeValidator.new('String', ["API", "SYSTEM", "PROD_SUPPORT"])
    ...
    def reason_code=(reason_code)
      validator = EnumAttributeValidator.new('String', ["R01", "R02", "R03", "R04", "R06", "R08", "R09", "R10", "R11", "R14", "R15", "R16", "R17", "R18", "R23", "R24", "R29"])
    ...
    def hash
      [token, channel, idempotent_hash, direct_deposit_token, state, reason_code, reason].hash
    end
