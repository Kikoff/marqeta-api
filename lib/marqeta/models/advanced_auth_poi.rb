  ...
  class AdvancedAuthPoi
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::AdvancedAuthPoi` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::AdvancedAuthPoi`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      moto_indicator_validator = EnumAttributeValidator.new('String', ["UNKNOWN", "MANUAL", "RECURRING", "INSTALLMENT", "OTHERS", "EMPTY"])
    ...
    def moto_indicator=(moto_indicator)
      validator = EnumAttributeValidator.new('String', ["UNKNOWN", "MANUAL", "RECURRING", "INSTALLMENT", "OTHERS", "EMPTY"])
    ...
    def hash
      [other, ecommerce, atm, moto, moto_indicator].hash
    end
