  ...
  class AuthRequestModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::AuthRequestModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::AuthRequestModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def pin=(pin)
      if !pin.nil? && pin.to_s.length > 50
        fail ArgumentError, 'invalid value for "pin", the character length must be smaller than or equal to 50.'
      end
    ...
    def hash
      [network_fees, webhook, card_token, amount, cash_back_amount, mid, is_pre_auth, pin, card_options, card_acceptor, transaction_options].hash
    end
