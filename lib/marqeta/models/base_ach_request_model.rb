  ...
  class BaseAchRequestModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::BaseAchRequestModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::BaseAchRequestModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @name_on_account.to_s.length > 50
        invalid_properties.push('invalid value for "name_on_account", the character length must be smaller than or equal to 50.')
      end

      if @name_on_account.to_s.length < 1
        invalid_properties.push('invalid value for "name_on_account", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @name_on_account.to_s.length > 50
      return false if @name_on_account.to_s.length < 1
      return false if @account_type.nil?
      account_type_validator = EnumAttributeValidator.new('String', ["checking", "savings", "corporate", "loan"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] name_on_account Value to be assigned
    def name_on_account=(name_on_account)
      if name_on_account.nil?
        fail ArgumentError, 'name_on_account cannot be nil'
      end

      if name_on_account.to_s.length > 50
        fail ArgumentError, 'invalid value for "name_on_account", the character length must be smaller than or equal to 50.'
      end

      if name_on_account.to_s.length < 1
        fail ArgumentError, 'invalid value for "name_on_account", the character length must be great than or equal to 1.'
      end
    ...
    def account_type=(account_type)
      validator = EnumAttributeValidator.new('String', ["checking", "savings", "corporate", "loan"])
    ...
    def hash
      [token, account_number, routing_number, name_on_account, account_type, bank_name, verification_override, verification_notes, is_default_account].hash
    end
