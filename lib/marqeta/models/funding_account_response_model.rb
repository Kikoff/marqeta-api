...
module Marqeta
  class FundingAccountResponseModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::FundingAccountResponseModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::FundingAccountResponseModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_time, last_modified_time, type, token, account_suffix, account_type, active, is_default_account, exp_date, verification_status, date_verified, user_token, business_token, name_on_account, date_sent_for_verification, verification_override, verification_notes].hash
    end
