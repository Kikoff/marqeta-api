  ...
  class StoreUpdateModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::StoreUpdateModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::StoreUpdateModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, active, contact, contact_email, longitude, latitude, address1, address2, city, state, province, zip, phone, country, token, partial_auth_flag, mid, network_mid, merchant_token, partial_approval_capable, keyed_auth_cvv_enforced].hash
    end
