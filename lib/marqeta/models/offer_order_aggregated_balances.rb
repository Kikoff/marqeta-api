  ...
  class OfferOrderAggregatedBalances
    ...
    def self.openapi_types
      {
        ...
        :'balances' => :'Hash<String, OfferOrderAggregatedBalances>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Marqeta::OfferOrderAggregatedBalances` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Marqeta::OfferOrderAggregatedBalances`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [currency_code, ledger_balance, available_balance, credit_balance, cached_balance, pending_credits, impacted_amount, balances, last_updated_time].hash
    end
